...
namespace OpenGL
{
	class Scene
	{
		...
		void drawSceneObjects(const Framebuffer* framebuffer = nullptr);
	...
	public:
		...
		DirectionalLight* setDirectionalLight(glm::vec3 direction, float diffuseIntensity, glm::vec3 diffuseColor = glm::vec3(1), glm::vec3 specularColor = glm::vec3(1));
		PointLight* addPointLight(glm::vec3 position, float power, float quadraticFactor, float linearFactor, float constantFactor = 1.f, glm::vec3 diffuseColor = glm::vec3(1), glm::vec3 specularColor = glm::vec3(1));
		SpotLight* addSpotLight(glm::vec3 position, glm::vec3 direction, float innerAngleInDegrees, float outerAngleInDegrees, float power, float quadraticFactor, float linearFactor, float constantFactor = 1.f, glm::vec3 diffuseColor = glm::vec3(1), glm::vec3 specularColor = glm::vec3(1));
		glm::vec3 getAmbientLightColor();
		float getAmbientLightIntensity();
		const DirectionalLight& getDirectionalLights() const	{ return *directionalLight; }
